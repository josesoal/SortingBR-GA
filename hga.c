/*
 ============================================================================
 Project    : Hybrid Genetic Algorithm for the SUPBR Problem
 File       : hga.c
...
static int start_hga(parameters *params, int *num_eval_fit_function, int *num_used_generations);
...
int main(int argc, char **argv){
    ...
    score = start_hga(&params, &num_eval_fit_function, &num_used_generations);
...
static int start_hga(parameters *params, int *num_eval_fit_function, int *num_used_generations){
    ...
    /***START HYBRID_GA**/
    generation_number = 1;
    int revs = improve_permutation(&perm);
    ...
    if (params->print_mode == BEST_BY_GENERATION)
        printf("%d ",best_solution + revs); //print best solution by generation
    ...
    if (best_solution + revs <= params->number_reversals)
        start_breeding = FALSE;
    ...
    best_solution = best_solution + revs;
